 ...
 *  - DB products: MongoDB ObjectId is short hex → append as /products/my-slug-64abc123
 *  - Static products: id === slug (long readable string) → just /products/my-slug
 */
function productUrl(p: Product): string {
  if (p.id === p.slug) return `/products/${p.slug}`;
...
export function ProductCard({ product: p }: { product: Product }) {
  ...
  const imgSrc = p.images[0] && !imgErr ? p.images[0] : null;
