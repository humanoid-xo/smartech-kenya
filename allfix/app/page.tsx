...
const SOCIAL = [
  ...
  { href: 'https://www.instagram.com/smartech_kenya?igsh=MXdpdTJldzU3eHQ2aQ%3D%3D&utm_source=qr', label: 'Instagram' },
  { href: 'https://tiktok.com/@smartech_kenya',                                               label: 'TikTok'    },
];
...
export default async function HomePage() {
  ...
  return (
    <div className="bg-cream">
      ...
      {/* ══ HERO — full viewport, no gap under header ══════════════════════ */}
      <section className="relative overflow-hidden bg-ink" style={{ minHeight: 'calc(100vh - 116px)' }}>
        <div className="max-w-[1320px] mx-auto px-6 grid lg:grid-cols-2 gap-0 items-center h-full"
          style={{ minHeight: 'calc(100vh - 116px)' }}>

          {/* ── LEFT: copy ── */}
          <div className="py-16 lg:py-20 lg:pr-12 z-10 relative">
          ...
          <div className="hidden lg:grid grid-cols-2 gap-3 py-8 pl-4"
          ...
          {[
            {
              ...
              img: 'https://images.unsplash.com/photo-1584568694244-14fbdf83bd30?w=700&q=80&auto=format&fit=crop',
            },
            {
              label: 'Tech Deals', href: '/products?category=SMARTPHONES',
              headline: 'Latest Phones\n& Laptops',
              sub: 'Samsung, HP & top brands',
              img: 'https://images.unsplash.com/photo-1496181133206-80ce9b88a853?w=700&q=80&auto=format&fit=crop',
            },
          ].map(b => (
            <Link key={b.label} href={b.href}
              className="group relative rounded-2xl overflow-hidden bg-ink flex items-center hover:-translate-y-0.5 transition-all duration-300"
              ...
              <div className="relative p-7 lg:p-8 z-10 max-w-[280px]">
                <p className="text-[9px] font-bold tracking-[0.18em] uppercase mb-2.5 text-cream/40">{b.label}</p>
                ...
                <p className="text-xs mb-5 font-medium text-cream/45">{b.sub}</p>
                <span className="inline-flex items-center gap-2 text-xs font-bold tracking-wide text-[#F97316]">
                  Shop Now
        ...
        <section className="py-16 px-6 bg-ink">
          <div className="max-w-[1320px] mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {kitchen.map((p: any) => <ProductCard key={p.id} product={p}/>)}
            </div>
          </div>
        </section>
      )}
        ...
        <div className="max-w-[1320px] mx-auto">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={f.n} className="p-6 rounded-2xl border border-cream-warm bg-white hover:shadow-sm transition-all duration-200">
                <p className="font-display text-[2.2rem] font-light mb-4 leading-none text-cream-muted">{f.n}</p>
      ...
      <section className="py-12 px-6 border-t border-cream-warm/60 bg-cream">
        <div className="max-w-[1320px] mx-auto">
          <p className="text-center text-[9px] font-bold tracking-[0.22em] uppercase text-cream-muted mb-8">
            Authorised brands
          </p>
      ...
      <section className="py-20 px-6 bg-ink">
        <div className="max-w-[1320px] mx-auto grid md:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div>
            <p className="text-[9.5px] font-bold tracking-[0.18em] uppercase mb-5 text-[#F97316]">Stay ahead</p>
            <h2 className="font-display text-cream mb-4"
              style={{ fontSize: 'clamp(1.9rem,3.8vw,2.9rem)', fontWeight: 400 }}>
              Get the best deals first
            </h2>
            <p className="text-sm leading-relaxed max-w-[320px] text-cream/45">
          ...
          <div>
            <form className="flex flex-col sm:flex-row gap-2.5 mb-3">
              <input type="email" placeholder="your@email.com" required
                className="flex-1 px-5 py-3.5 rounded-full border text-sm text-cream placeholder-cream/25 bg-transparent focus:outline-none transition-all"
                style={{ borderColor: 'rgba(245,240,232,0.14)' }}/>
              <button type="submit"
                className="px-6 py-3.5 rounded-full bg-[#F97316] text-white text-sm font-bold shrink-0 whitespace-nowrap hover:bg-[#EA6A05] transition-all">
                Subscribe
              </button>
            </form>
            <p className="text-xs text-cream/25">Unsubscribe any time.</p>
          </div>
        </div>
      </section>
    ...
    <div className="flex items-end justify-between mb-10">
      <div>
        <p className="text-[9.5px] font-bold tracking-[0.16em] uppercase mb-2.5"
          style={{ color: dark ? 'rgba(245,240,232,0.38)' : '#8B5A1A' }}>
          {label}
        </p>
      ...
      <Link href={href}
        className={[
          ...
          dark ? 'text-cream/35 hover:text-cream/65' : 'text-ink/40 hover:text-ink',
        ].join(' ')}>
        {cta}
