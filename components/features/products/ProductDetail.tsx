...
export function ProductDetail({ product }: { product: Product }) {
  ...
  return (
    <div className="bg-white rounded-3xl border border-cream-warm overflow-hidden">
      <div className="grid md:grid-cols-2 gap-0">

        {/* Images */}
        <div className="p-8 bg-cream-warm/50 border-r border-cream-warm">
        ...
        <div className="p-8 lg:p-10 flex flex-col">
          <div className="text-[10px] font-bold text-cream-muted uppercase tracking-widest mb-2">{product.brand}</div>
          <h1 className="font-display text-2xl lg:text-3xl text-ink leading-tight mb-4">{product.name}</h1>
          ...
          {product.avgRating > 0 && (
            <div className="flex items-center gap-2 mb-5">
              ...
              <span className="text-sm text-ink-faint">{product.avgRating.toFixed(1)} ({product.reviewCount} reviews)</span>
            </div>
          )}

          {/* Price */}
          <div className="flex items-baseline gap-3 mb-6">
            <span className="text-3xl font-bold text-ink">KES {product.price.toLocaleString()}</span>
            {product.comparePrice && (
              <span className="text-cream-muted line-through text-lg">KES {product.comparePrice.toLocaleString()}</span>
            )}
          </div>

          <p className="text-ink-muted text-sm leading-relaxed mb-8">{product.description}</p>
          ...
          {product.stock > 0 && (
            <div className="flex gap-3 mb-6">
              <div className="flex items-center gap-2 bg-cream-warm/50 rounded-xl px-3 py-2 border border-cream-warm">
                <button
          ...
          {/* Seller */}
          <div className="pt-5 mt-auto border-t border-cream-warm flex items-center gap-2">
            <div className="w-7 h-7 bg-cream-warm rounded-full flex items-center justify-center text-ink-faint text-xs font-bold">
              {product.seller.name[0]}
            </div>
            <span className="text-xs text-cream-muted">Sold by <span className="font-medium text-ink-muted">{product.seller.name}</span></span>
          </div>
        </div>
      </div>
      ...
      {product.specifications && Object.keys(product.specifications).length > 0 && (
        <div className="border-t border-cream-warm p-8 lg:p-10">
          <h2 className="font-semibold text-ink mb-5">Specifications</h2>
          <div className="grid sm:grid-cols-2 gap-3">
            {Object.entries(product.specifications as Record<string, string>).map(([k, v]) => (
              <div key={k} className="flex gap-3 py-2.5 border-b border-gray-50">
                <span className="text-ink-faint text-sm w-32 shrink-0">{k}</span>
                <span className="text-ink text-sm font-medium">{String(v)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {product.reviews.length > 0 && (
        <div className="border-t border-cream-warm p-8 lg:p-10">
          <h2 className="font-semibold text-ink mb-6">
            Customer Reviews
            <span className="ml-2 text-cream-muted font-normal">({product.reviewCount})</span>
          </h2>
          <div className="space-y-5">
            {product.reviews.slice(0, 5).map((review) => (
              <div key={review.id} className="border-b border-gray-50 pb-5 last:border-0">
                ...
                {review.comment && <p className="text-ink-muted text-sm leading-relaxed">{review.comment}</p>}
              </div>
            ))}
          </div>
        </div>
