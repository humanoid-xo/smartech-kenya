#!/usr/bin/env python3
"""
populate_fixes.py — creates the 3 missing files in fixes/
Run from your repo root:
    python3 populate_fixes.py
Then:
    python3 deploy.py
"""
...
def write(rel_path, content):
    path = os.path.join(REPO, 'fixes', rel_path)
    os.makedirs(os.path.dirname(path), exist_ok=True)
    with open(path, 'w') as f:
        f.write(content)
    print(f'  ✔  fixes/{rel_path}')

def copy_from_repo(src_rel):
    """Copy a file that already exists in the repo into fixes/."""
    src = os.path.join(REPO, src_rel)
    dst = os.path.join(REPO, 'fixes', src_rel)
    os.makedirs(os.path.dirname(dst), exist_ok=True)
    shutil.copy2(src, dst)
    print(f'  ✔  fixes/{src_rel}  (copied from repo)')

# ──────────────────────────────────────────────────────────────
# 1.  app/layout.tsx  — wraps children with <Providers>
# ──────────────────────────────────────────────────────────────
LAYOUT = '''\
...
export const metadata: Metadata = {
  ...
  description: "Premium Tech & Gadgets Store | Nairobi, Kenya",
};
...
# ──────────────────────────────────────────────────────────────
# 2.  app/page.tsx  — homepage with STATIC_PRODUCTS fallback
# ──────────────────────────────────────────────────────────────
PAGE = '''\
...
function withMeta(p: any) {
  return { ...p, avgRating: 0, reviewCount: 0 };
}
...
async function getFeatured() {
  try {
    ...
    return STATIC_PRODUCTS.slice(0, 8).map(withMeta);
  }
}
...
async function getLatest() {
  try {
    ...
    return STATIC_PRODUCTS.slice(0, 4).map(withMeta);
  }
}
...
async function getKitchen() {
  try {
    ...
    return STATIC_PRODUCTS.filter((p: any) => p.category === 'KITCHEN').map(withMeta);
  }
}
...
# ──────────────────────────────────────────────────────────────
print('─' * 56)
print('  Populating fixes/ with missing files')
print('─' * 56)

write('app/layout.tsx', LAYOUT)
write('app/page.tsx',   PAGE)

# admin/page.tsx already exists in the repo and is already the
# polished version — just copy it over.
admin_src = os.path.join(REPO, 'app', 'admin', 'page.tsx')
if os.path.exists(admin_src):
    copy_from_repo('app/admin/page.tsx')
else:
    print('  ⚠  app/admin/page.tsx not found in repo — skipping')
