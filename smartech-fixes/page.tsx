...
async function getFeatured() {
  try {
    ...
    // fallback: use static featured products
    return STATIC_PRODUCTS.filter(p => p.isFeatured).slice(0, 8) as any[];
  } catch {
    return STATIC_PRODUCTS.filter(p => p.isFeatured).slice(0, 8) as any[];
  }
}
...
async function getLatest() {
  try {
    ...
    if (mapped.length > 0) return mapped;
    return STATIC_PRODUCTS.slice(0, 4) as any[];
  } catch {
    return STATIC_PRODUCTS.slice(0, 4) as any[];
  }
}
...
async function getKitchen() {
  try {
    ...
    if (mapped.length > 0) return mapped;
    return STATIC_PRODUCTS.filter(p => p.category === 'KITCHEN').slice(0, 4) as any[];
  } catch {
    return STATIC_PRODUCTS.filter(p => p.category === 'KITCHEN').slice(0, 4) as any[];
  }
}
