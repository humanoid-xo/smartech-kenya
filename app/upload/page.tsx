...
export default function UploadPage() {
  ...
  if (!authed) return (
    <div style={{ minHeight:'100vh', background:'#0A0A14', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'system-ui' }}>
      <div style={{ width:320, textAlign:'center' }}>
        <p style={{ color:'#F8F9FB', fontSize:22, fontWeight:600, marginBottom:8 }}>Smartech Upload</p>
        ...
        <input type="password" placeholder="Admin password" value={secret} onChange={e => setSecret(e.target.value)}
          ...
          style={{ width:'100%', padding:'12px 16px', borderRadius:12, border:'1px solid rgba(255,255,255,0.12)', background:'rgba(255,255,255,0.06)', color:'#F8F9FB', fontSize:14, boxSizing:'border-box', marginBottom:10 }}/>
        <button onClick={() => setAuthed(true)} disabled={!secret}
          style={{ width:'100%', padding:'13px', borderRadius:12, background:'#003A7A', color:'#F8F9FB', fontWeight:700, fontSize:14, border:'none', cursor:'pointer' }}>
          Sign In
        </button>
      </div>
    </div>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F8F9FB', fontFamily:'system-ui', padding:'40px 20px' }}>
      <div style={{ maxWidth:520, margin:'0 auto' }}>
        <p style={{ fontSize:22, fontWeight:700, color:'#0A0A14', marginBottom:4 }}>Add Product</p>
        ...
        {status && (
          <div style={{ padding:'12px 16px', borderRadius:10, marginBottom:16, fontSize:13, fontWeight:500,
            ...
            color: status.startsWith('✓') ? '#166534' : status.startsWith('Error') ? '#dc2626' : '#003A7A',
        ...
        <button onClick={submit} disabled={!form.name || !form.price || !img}
          style={{ width:'100%', padding:'15px', borderRadius:12, background:'#0A0A14', color:'#F8F9FB',
        ...
        {done.length > 0 && (
          <div style={{ marginTop:28 }}>
            ...
            {done.map((d,i) => (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 14px', borderRadius:10, background:'#fff', border:'1px solid #E5E7EB', marginBottom:8 }}>
                {d.url && <img src={d.url} style={{ width:44, height:44, objectFit:'contain', borderRadius:8, background:'#F8F9FB' }}/>}
                <span style={{ fontSize:13, fontWeight:600, color:'#0A0A14' }}>{d.name}</span>
