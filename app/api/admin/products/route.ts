...
export async function POST(req: NextRequest) {
  try {
    ...
    const { secret, name, brand, sku, category, price, comparePrice, stock, description, subcategory, imageBase64 } = body;
    ...
    if (!name || !brand || !sku || !category || price === undefined) {
      return NextResponse.json({ error: 'name, brand, sku, category and price are required' }, { status: 400 });
    }
    ...
    if (existing) {
      return NextResponse.json({ error: `SKU "${sku}" is already in use by "${existing.name}"` }, { status: 409 });
    }
