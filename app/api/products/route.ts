...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!result.success) {
      return NextResponse.json({ success: false, error: 'Validation failed', errors: result.error.flatten().fieldErrors }, { status: 400 });
    }
    ...
    const product = await prisma.product.create({
      data: { ...result.data, slug: `${slug}-${Date.now()}`, sellerId: (session.user as any).id },
    });
