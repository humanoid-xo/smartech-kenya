...
export async function GET(request: NextRequest) {
  try {
    ...
    const productsWithRatings = products.map((p: any) => ({ ...p, avgRating: p.reviews.length > 0 ? p.reviews.reduce((sum: number, r: any) => sum + r.rating, 0) / p.reviews.length : 0, reviewCount: p.reviews.length }));
